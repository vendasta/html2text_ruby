...
class Html2Text
  ...
  def remove_leading_and_trailing_whitespace(text)
    # String#gsub! returns nil if no substition was performed, so these calls can't be chained.  Use
    # #tap so the text is returned regardless of the substition.
    text.tap do |s|
      s.gsub!(/[ \t]*\n[ \t]*/im, "\n")
      s.gsub!(/ *\t */im, "\t")
    end
  ...
  def remove_unnecessary_empty_lines(text)
    # Use #tap so the text is returned regardless of the substition.
    text.tap do |s|
      s.gsub!(/\n\n\n*/im, "\n\n")
    end
  end
